...
async function useFirebaseAuthState(sessionId) {
    ...
    if (!creds) {
        creds = initAuthCreds();
        await ref.child('creds').set(cleanData(creds)); // 🔥 FIX
    }
    ...
    return {
                ...
                set: async (data) => {
                    for (const type in data) {
                        for (const id in data[type]) {
                            ...
                            if (value) {
                                await ref.child(`keys/${type}-${id}`)
                                    .set(cleanData(value)); // 🔥 FIX
                            } else {
        ...
        saveCreds: async (newCreds) => {
            await ref.child('creds').set(cleanData(newCreds)); // 🔥 FIX
        }
    };
}
...
async function createBot(sessionId) {
    ...
    const sock = makeWASocket({
        ...
        browser: ['MAINUL-X BOT', 'Chrome', '120']
    });
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (reason !== DisconnectReason.loggedOut) {
                ...
                setTimeout(() => createBot(sessionId), 3000);
            } else {
...
app.post('/pair', async (req, res) => {
    ...
    try {
        ...
        console.log(`📱 Requesting pairing: ${cleanNumber}`);
